...
*   Возможно, этот релиз уже записан в `github_releases_state.json`. Чтобы принудительно получить повторное саммари и уведомление для конкретного репозитория, **отредактируйте `github_releases_state.json` и установите `id` в `0`, а `etag` в `null`** для этого репозитория. Например:
    ```json
    "НазваниеВашегоРепо": {
      "id": 0,
      "etag": null
    ...
    ```
    Это заставит бота "забыть" последнее состояние и повторно обработать последний релиз.
